...
function dateFilter(req, defaultDias = 30) {
...
async function pedidosEnRango(req, res, defaultDias) {
...
router.get('/kpi', async (req, res) => {
    try {
        ...
        const { data: ingresosDelPeriodo } = await supabase_1.supabase
            .from('pedidos')
            .select('total')
            .eq('estado', 'Pagado')
            .gte('fecha', desde)
        ...
        const { count: totalCarritos } = await supabase_1.supabase
            .from('pedidos')
            .select('*', { count: 'exact', head: true })
            .eq('estado', 'Carrito');
        const { count: totalPagadosTodo } = await supabase_1.supabase
            .from('pedidos')
            .select('*', { count: 'exact', head: true })
            .eq('estado', 'Pagado');
        ...
        const { data: pagosPeriodo } = await supabase_1.supabase
            .from('pagos')
            .select('estado')
...
router.get('/errores-pago', async (req, res) => {
    try {
        const { desde, hasta } = dateFilter(req);
...
router.get('/auditoria-stock', async (req, res) => {
    try {
...
router.get('/tendencia-ventas', async (req, res) => {
    try {
        const { desde, hasta } = dateFilter(req);
...
router.get('/tendencia-clientes', async (req, res) => {
    try {
        const { desde, hasta } = dateFilter(req);
        const desdeDate = new Date(desde);
        const hastaDate = new Date(hasta);
        ...
        for (const u of data || []) {
            ...
            if (clientesPorDia[dia] !== undefined)
                clientesPorDia[dia]++;
        }
...
//# sourceMappingURL=analista.js.map