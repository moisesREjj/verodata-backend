...
const ERRORES_SIMULADOS: Record<string, { codigo: string; mensaje: string }> = {
...
function simularPago(metodoPago: string, datosPago: any): {
  exitoso: boolean
  ...
  if (metodoPago === 'Yape' || metodoPago === 'Plin') {
    ...
    return {
      exitoso: true,
      transaccionId: `TXN-${uuidv4().slice(0, 8).toUpperCase()}`,
    }
  }
...
router.get('/', authenticate, authorize('ROLE_ADMIN', 'ROLE_ANALISTA'), async (req: Request, res: Response) => {
  try {
    const { estado, metodo_pago, desde, hasta, page, limit } = req.query
    let query = supabase
      .from('pagos')
      .select('*, pedidos!inner(codigo, id_usuario, usuarios!inner(nombre, email))', { count: 'exact' })
...
router.post('/procesar', authenticate, async (req: Request, res: Response) => {
  try {
