...
router.get('/resumen', async (_req: Request, res: Response) => {
  try {
    ...
    const { count: clientesPrevios } = await supabase
      .from('usuarios')
      .select('*', { count: 'exact', head: true })
    ...
    const alertasStock = (productos || []).map((p: any) => ({
      ...p,
    ...
    const catVentas: Record<string, number> = {}
    ...
    const { count: carrito } = await supabase
      .from('pedidos')
      .select('*', { count: 'exact', head: true })
      .eq('estado', 'Carrito')
    ...
    const { count: enviado } = await supabase
      .from('pedidos')
      .select('*', { count: 'exact', head: true })
      .eq('estado', 'Enviado')
...
router.get('/notificaciones', async (_req: Request, res: Response) => {
  try {
    const now = new Date()
